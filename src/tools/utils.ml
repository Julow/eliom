...
let get_kind k = match k with Some k -> k | None -> (!kind : kind :> [> kind])
...
let has_package name =
  try
    ignore (Findlib.package_directory name);
    true
...
let prefix_type_dir name =
  match !type_dir with "" | "." -> name | d -> d ^ "/" ^ name

let get_type_file file =
  match !type_file with
  | Some f -> f
  | None -> prefix_type_dir (chop_extension_if_any file ^ !server_types_file_ext)

let impl_intf_opt = function `Impl -> "-impl" | `Intf -> "-intf"
...
let get_ppopts ~impl_intf file =
  match !pp_mode with
  | `Camlp4 -> type_opt impl_intf file @ !ppopt @ [impl_intf_opt impl_intf]
  | `Ppx -> type_opt impl_intf file @ !ppopt

let preprocess_opt ?(ocaml = false) ?kind opts =
  match !pp_mode with
  | `Camlp4 -> ["-pp"; get_pp [] ^ " " ^ String.concat " " opts]
  | `Ppx when ocaml -> []
  | `Ppx ->
      let pkg =
        match simplify_kind ?kind () with
