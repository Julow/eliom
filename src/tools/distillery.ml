...
let copy_file ?(env = []) ?(preds = []) src_name dst_name =
  ...
  | Preprocessing_error msg -> eprintf "%s.\n%!" msg
  | File_error msg -> eprintf "%s.\n%!" msg

let expand_dest_path ~name ~dest_dir s =
  Str.(global_replace (regexp (quote "PROJECT_NAME")) name s)
  |> Str.(split (regexp "!"))
  |> join_path |> Filename.concat dest_dir
...
let check_reserve_project_name project_name template =
  ...
  |> lines_of_file |> List.mem project_name
...
let main () =
  ...
  then ()
    ...
    then
      ...
      create_project ~without_asking:!without_asking ~name ~env ~source_dir
        ~dest_dir ()
