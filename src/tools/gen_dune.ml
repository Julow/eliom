...
(* let handle_file_server nm = if Filename.check_suffix nm ".eliom" then let nm
   = Filename.chop_suffix nm ".eliom" in Printf.printf "(rule (target
   %s.type_mli) (deps %s.eliom)\n\ \ (action\n\ \ (progn\n\ \ (ignore-stdout
   (echo %%{cmo:%s}))\n\ \ (with-stdout-to %%{target}\n\ \ (pipe-stdout\n\ \
   (run ocamlc %%{read-lines:includes} -I .pervasives.objs/byte -I
   .bs.eobjs/byte -i -ppx \"%%{exe:ppx/eliom_ppx_type.exe} -as-ppx --prefix Bs_
   --suffix _i18n --default-module Bs_i18n\" %s -impl %%{deps})\n\ \ (run sed -e
   \"s$/[1-9][0-9]*$$g\" -e \"s/_\\\\[\\\\([<>]\\\\)/[\\\\1/g\" -e
   \"s/'\\\\(_[a-z0-9_]*\\\\)/'eliom_inferred_type_\\\\1/g\"))))))\n" nm nm nm
